...
import { ExtendedError, TransportCryptoManager } from '@ts-core/common';
...
export class CryptoLedgerSignHandler extends TransportCommandAsyncHandler<ICryptoLedgerSignDto, ISignature, CryptoLedgerSignCommand> {
    ...
    protected async execute(params: ICryptoLedgerSignDto): Promise<ISignature> {
        let user = await UserEntity.createQueryBuilder('user')
            .leftJoinAndSelect('user.cryptoKey', 'userCryptoKey')
            .where(`user.ledgerUid  = :ledgerUid`, { ledgerUid: params.uid })
            .getOne();
        ...
        return TransportCryptoManager.sign(params.command, manager, key);
    }
}
